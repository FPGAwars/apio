...
class SconsHandler:
    ...
    def _register_apio_upload_target(self, synth_srcs):
    ...
    def _register_apio_report_target(self, synth_srcs):
    ...
    def _register_apio_graph_target(
        self,
        synth_srcs,
    ):
    ...
    def _register_apio_lint_target(self, synth_srcs, test_srcs):
    ...
    def execute(self):
        ...
        # -- python 3.9.
        if target == "build":
            self._register_apio_build_target(synth_srcs)

        elif target == "upload":
            self._register_apio_upload_target(synth_srcs)

        elif target == "report":
            self._register_apio_report_target(synth_srcs)

        elif target == "graph":
            self._register_apio_graph_target(synth_srcs)

        elif target == "sim":
            self._register_apio_sim_target(synth_srcs, test_srcs)

        elif target == "test":
            self._register_apio_test_target(synth_srcs, test_srcs)
