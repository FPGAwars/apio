...
from apio.common import build_report
...
The command 'apio api get-build-report' provides utilization and max \
clock information from a built project. The information is extracted \
from the file 'hardware.pnr' that is generated by Apio when building \
the project.
...
Examples:[code]
  apio api get-build-report               # Report for default env
  apio api get-build-report -e env1       # Report for specified env
  apio api get-build-report -p foo/bar    # Project in another dir
  apio api get-build-report -o apio.json  # Write to a file[/code]
"""
...
@click.command(
    ...
    cls=ApioCommand,
    short_help="Get project build information.",
...
def _get_build_report_cli(
    *,
    ...
    """Implements the 'apio apio get-build-report' command."""
    ...
    # -- The build process generates this report file.
    pnr_json_file = apio_ctx.env_build_path / "hardware.pnr"
    ...
    section_dict["clocks"] = clocks_dict
...
def _get_project_cli(
    *,
    ...
    programmer_dict = {
        "id": pr.programmer_id,
        "is-custom": apio_ctx.definitions.is_custom_programmer(
            pr.programmer_id
        ),
    }
...
# ------ apio api get-packages

# -- Text in the rich-text format of the python rich library.
APIO_API_GET_PACKAGES_HELP = """
The command 'apio api get-packages' exports information about  \
the apio packages on the current platform as a JSON foc. Note that  \
the set of apio packages used depends on the platform used as some \
packages may be platform specific.
...
Examples:[code]
  apio api get-packages               # Write to stdout
  apio api get-packages -o apio.json  # Write to a file[/code]
"""
...
@click.command(
    name="get-packages",
    cls=ApioCommand,
    short_help="Retrieve apio packages information.",
    help=APIO_API_GET_PACKAGES_HELP,
)
# @click.pass_context
@timestamp_option
@output_option
@options.force_option_gen(short_help="Overwrite output file.")
def _get_packages_cli(
    *,
    # Options
    timestamp: str,
    output: str,
    ...
    """Implements the 'apio apio get-packages' command."""
