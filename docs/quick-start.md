...
# Apio CLI quick start

In this page, we will go through the steps of creating, validating, and uploading a design to an FPGA board
using Apio CLI.  We will use the Alhambra-ii FPGA board, but the process is the same for all supported boards.
...
We are in luck; the device's manufacturer and description strings are listed correctly, which means that the device is accessible to Apio CLI and doesn't require an additional driver. We are ready to program the FPGA.
...
This concludes the Apio CLI quick start guide, we suggest continuing to the [Video tutorial](video-tutorial.md)
