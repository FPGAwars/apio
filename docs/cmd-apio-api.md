...
apio api get-build-report -o apio.json  # Write to a file
```
...
The command `apio api get-packages` exports information about the
apio packages on the current platform as a JSON foc. Note that the
set of apio packages used depends on the platform used as some
...
apio api get-packages -o apio.json  # Write to a file
```
